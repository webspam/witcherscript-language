...
`parent` and `virtual_parent` resolve against a state's owner class, so they are only valid inside a state method. A use in a plain class, struct, or free function is flagged. A `@wrapMethod` (etc.) function counts as a state method when its short-name target resolves to a state, since the owner class is then in scope.
