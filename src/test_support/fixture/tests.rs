...
fn parses_single_file_with_cursor_and_span() {
    ...
    assert_eq!(f.files[0].uri, "file:///main.ws");
    assert_eq!(
        f.files[0].text,
        "function Foo() {}\nfunction Bar() { Foo(); }\n"
    );
    let (uri, pos) = f.cursor.clone().unwrap();
    assert_eq!(uri, "file:///main.ws");
    assert_eq!(
        pos,
        SourcePosition {
            line: 1,
    ...
    assert_eq!(span_uri, "file:///main.ws");
    assert_eq!(
        span,
        SourceRange {
...
fn parses_multi_file_fixture() {
    ...
    assert_eq!(f.files[0].uri, "file:///lib.ws");
    assert_eq!(f.files[0].text, "function A() {}\n");
    assert_eq!(f.files[1].uri, "file:///other.ws");
