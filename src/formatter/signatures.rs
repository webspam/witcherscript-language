...
impl Formatter<'_> {
    pub(super) fn format_func_params(&mut self, func_node: Node) {
        let params = self.child_of_kind(func_node, kinds::FUNC_PARAMS);
        ...
        let inner_nodes: Vec<Node> = self
            .child_of_kind(func_node, kinds::FUNC_PARAMS)
            .map(|fp| {
                let mut c = fp.walk();
                fp.children(&mut c)
        ...
        let rendered: Vec<String> = inner_nodes
            .iter()
            .map(|n| {
                if n.kind() == kinds::FUNC_PARAM_GROUP {
        ...
        let group_count = inner_nodes
            .iter()
            .filter(|n| n.kind() == kinds::FUNC_PARAM_GROUP)
            .count();
    ...
    fn return_type_suffix(&self, func_node: Node) -> String {
        ...
        for child in func_node.children(&mut c) {
            if child.kind() == kinds::FUNC_PARAMS {
                past_params = true;
            }
