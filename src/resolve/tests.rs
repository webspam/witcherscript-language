...
fn annotation_arg_completions_offers_classes_inside_parens() {
    ...
    let completions = super::annotation_arg_completions(
        &doc,
        &SymbolDb::new(&index, &WorkspaceIndex::default()),
...
fn after_wrap_method_stage1_offers_only_function_keyword() {
    ...
    let result = super::after_wrap_method_completions(
        &doc,
        &db,
        SourcePosition {
            line: 3,
    ...
    assert!(
        matches!(
            result,
            Some(super::AfterWrapMethodCompletions::FunctionKeyword)
        ),
...
fn after_wrap_method_stage2_offers_method_list_after_function_keyword() {
    ...
    let methods = match result {
        Some(super::AfterWrapMethodCompletions::MethodList(m)) => m,
...
fn after_wrap_method_stage2_does_not_walk_inheritance() {
    ...
    let doc_base = parse_document(source_base).expect("parse");
    let doc = parse_document(source).expect("parse");
    let mut index = WorkspaceIndex::default();
    ...
    let methods = match super::after_wrap_method_completions(
        &doc,
        &db,
        SourcePosition {
            line: 4,
...
fn wrap_method_snippet_plain_params() {
    ...
    let snippet = super::wrap_method_snippet(&method, &db);
    assert_eq!(
        snippet,
