...
| Code actions | Quick fix for `base_script_conflict`: marks the conflicting directory as a legacy override directory. Refactor on a `switch`/`case`/`default` keyword: collapse every case onto one line, or expand each onto its own line |
