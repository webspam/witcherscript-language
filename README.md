...
| Inlay hints | Parameter-name labels at call sites (`out` parameters shown as `out name:`); honours the requested range and skips a non-`out` argument that already spells the parameter name. On by default; toggle with `witcherscript.inlayHints.parameterNames` |
...
| `witcherscript.inlayHints.parameterNames` | `boolean` | `true` | Show parameter-name inlay hints at call sites, labelling each argument with the parameter it fills. `out` parameters are shown as `out name:`. A hint is omitted when a non-`out` argument already spells the parameter name. Live-switchable. |
